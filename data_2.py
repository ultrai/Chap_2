...
for stack in range(Contours.shape[0]):
    Contours[stack,:,:] = ndimage.grey_dilation(np.uint(Contours[stack,:,:]), size=(2,2))
...
with h5py.File("train.hdf5", "w") as f:
     ...
     dset = f.create_dataset("L1", data = L1, dtype='float32')
     dset = f.create_dataset("L2", data = L2, dtype='float32')
     dset = f.create_dataset("L3", data = L3, dtype='float32')
     dset = f.create_dataset("L4", data = L4, dtype='float32')
     dset = f.create_dataset("L5", data = L5, dtype='float32')
     dset = f.create_dataset("L6", data = L6, dtype='float32')
     dset = f.create_dataset("L7", data = L7, dtype='float32')
     dset = f.create_dataset("L8", data = L8, dtype='float32')
