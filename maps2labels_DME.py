...
for x in range(len(ls)):
    ...
    if x==0:
        ...
        Labels_AN = np.concatenate((Labels_AN,L_AN),axis=2)
        Labels_AD = np.concatenate((Labels_AD,L_AD),axis=2)
        Contours_AN = np.concatenate((Contours_AN,C_AN),axis=2)
        Contours_AD = np.concatenate((Contours_AD,C_AD),axis=2)
Labels = Labels[:,120:651,:]
Contours = Contours[:,120:651,:]
Labels1 = Labels1[:,120:651,:]
Contours1 = Contours1[:,120:651,:]
Labels2 = Labels2[:,120:651,:]
Contours2 = Contours2[:,120:651,:]
...
import scipy.io as sio
sio.savemat('Data.mat', {'Images': Images, 'Label': Labels, 'Contour': Contours, 'Label1': Labels1, 'Contour1': Contours1, 'Label2': Labels2, 'Contour2': Contours2, 'Label_AN': Labels_AN, 'Contour_AN': Contours_AN, 'Label_AD': Labels_AD, 'Contour_AD': Contours_AD})
