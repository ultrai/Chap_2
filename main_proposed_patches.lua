...
local function createModel(opt)
	...
	local U4 = ConvLayers(nfilt*grow[5],nfilt*grow[4])(Join(1,3)({ D4, ReLU(true)(UpConvolution(nfilt*grow[5],nfilt*grow[4], 2,2,2,2)z(B)) }))
  ...
  local U = Convolution(nfilt*grow[1], nbClasses, 1,1)(U1)
	...
	local B_edge = cudnn.LogSoftMax()(ConvLayers(nfilt*grow[5],nbClasses)(B)) 
  
	local net = nn.Sequential()
	net:add(nn.gModule({input}, {U,B_edge}))
	return net
end
...
criteria1:cuda()
criteria2 = nn.ClassNLLCriterion()
criteria2:cuda()
...
temp=Temp.label2
temp = temp:type('torch.FloatTensor')
Y_train2 = (torch.Tensor(temp:size(1),temp:size(3),temp:size(4)):copy(temp))[{{},{},{16},{16}}]
...
if (ee==1) then     
    ...
    for i = 1,X_test:size(1)-batch2,batch2 do
        ...
        oo,out_train[{{i,i+batch2},{},{}}] = torch.max(output[1]:float(),1)
        collectgarbage()
    end
    output = model:forward(X_test[{{X_test:size(1)-batch2,X_test:size(1)},{},{},{}}]:cuda())
    oo,out_train[{{X_test:size(1)-batch2,X_test:size(1)},{},{}}] = torch.max(output[1]:float(),1)
...
optimMethod = optim.adagrad--adam--adadelta--adam--cg--adagrad--adam
sys:tic()
train = {}
test = {}
neval = 0
